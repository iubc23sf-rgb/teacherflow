...
- ダッシュボード（タスクリスト・週表示カレンダー[月〜金]・月表示カレンダー[タスク期限/面談/行事バッジ付き]）
- タスク管理（追加・完了切替・削除、期限順/重要度順/科目別ソート）
- 時間割（自分用/担任クラス用の2種類。週表示・編集、科目/クラスの登録、セル単位での割り当て・クリア、空きコマ表示）
- 面談記録（生徒名・クラス・面談日・種別・メモの記録、ダッシュボードとも連動）
- 行事・部活・校務分掌の記録（予定名・日付・種別・メモ、ダッシュボードの月表示とも連動）
...
- 時間割の自動生成
- Googleカレンダーとの実際の同期処理（実装対象から除外。自前データベースの表示に置き換え済み）
