...
**構想から除外（2026-07-20）:** クラス管理・テスト成績管理は、生徒の個人情報を
扱うリスクが大きいためユーザー判断で実装対象・ダッシュボード表示から外した。
サイドバーからも削除済み。将来再検討する場合は個人情報保護方針を先に固めること。

## 次にやると良さそうなこと

1. AIチャット、PDF→ToDoを次フェーズとして実装（ユーザー指定）
